...
def test(model, args):
    ...
    for i, data in enumerate(testloader):
        ...
        if not os.path.exists(os.path.join(save_dir, 'result_NYUD')):
            os.mkdir(os.path.join(save_dir, 'result_NYUD'))
        cv2.imwrite(os.path.join(save_dir, 'result_NYUD', '%s'%x_path), fuse*255)
        print(x_path)
        all_t += time.time() - tm
    print(all_t)
